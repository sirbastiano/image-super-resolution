...
class Trainer:
    ...
    def _load_weights(self):
        ...
        if self.weights_generator:
            generator_weights_path = str(self.weights_generator)
            if os.path.exists(generator_weights_path):  # Check if the file exists
                print("Loading generator weights:", self.weights_generator)
                self.model.get_layer('generator').load_weights(generator_weights_path)
            else:
                print("Generator weights file does not exist:", generator_weights_path)


        if self.discriminator:
            if self.weights_discriminator:
                discriminator_weights_path = str(self.weights_discriminator)
                if os.path.exists(discriminator_weights_path):  # Check if the file exists
                    print("Loading discriminator weights:", self.weights_discriminator)
                    self.model.get_layer('discriminator').load_weights(discriminator_weights_path)
                    self.discriminator.model.load_weights(discriminator_weights_path)
                else:
                    print("Discriminator weights file does not exist:", discriminator_weights_path)
